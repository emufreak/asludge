param(
    ...
    [array]$paramsArray # Array containing width, height, startX, startY for each file
)
...
Get-ChildItem -Path . -Filter "$inputFileName*.png" | ForEach-Object -Process {
    ...
    $width = [int]$paramsArray[$counter * 4]
    $height = [int]$paramsArray[$counter * 4 + 1]
    $startX = [int]$paramsArray[$counter * 4 + 2]
    $startY = [int]$paramsArray[$counter * 4 + 3]
