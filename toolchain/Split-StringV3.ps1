...
# Function to split input string into substrings with width below 192 while keeping words intact
function Split-StringByWidth {
    ...
    foreach ($word in $words) {
        ...
        if (($currentWidth + $wordWidth) -gt $maxWidth) {
            $padding = [math]::Floor(($maxWidth - $currentWidth + 5) / 2)
            while ($padding -gt 5) {
                $currentString = " " + $currentString
                $padding -= 5
            }
            $result += $currentString.TrimEnd()
            $currentString = ""
            $currentWidth = 0
        }
